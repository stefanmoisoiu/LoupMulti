...
namespace Player.UI
{
    ...
    {
        [SerializeField] private Movement.Stamina stamina;
        ...
        protected override void UpdateAnyOwner()
        {
            ...
            for (int i = 0; i < _staminaSliders.Count; i++)
            {
                ...
                _staminaSliders[i].value =
                    Mathf.InverseLerp(lowerBound, upperBound, stamina.StaminaValue);
            }
            ...
            for (int i = 0; i < _staminaSliders.Count; i++)
            {
                float upperBound = stamina.StaminaPerPart * (i + 1);
                float lowerBound = stamina.StaminaPerPart * i;
                if (stamina.StaminaValue >= upperBound)
                {
                    _staminaFillImages[i].color = fullColor;
                }
                else if (stamina.InGrace(stamina.StaminaValue, lowerBound, upperBound))
                {
